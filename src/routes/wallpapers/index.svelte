<script context="module" lang="ts">
	export async function preload() {
		...
		const lookup: { data: [{ slug: string, name: string, mobile_asset: string }]} = await res.json();
		const wallpapers = lookup.data.map(object => {
			return {
				id: object.slug,
				name: object.name,
				preview: `https://cms.tale.me/assets/${object.mobile_asset}?width=256&height=256&quality=50`
			}
		});
